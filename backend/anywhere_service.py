...
class AnywhereService:
    ...
    async def search_anywhere(self, origin: str, months_ahead: int = 1, promo_code: str = None, max_price: float = None) -> List[Dict]:
        ...
        async def process_destination_with_progress(destination):
            nonlocal processed
            result = await self._find_cheapest_flight_full_power(origin, destination, months_ahead, promo_code, max_price)
            processed += 1
    ...
    async def _find_cheapest_flight_full_power(self, origin: str, destination: str, months_ahead: int = 1, promo_code: str = None, max_price: float = None) -> Optional[Dict]:
        ...
        try:
            ...
            flights_data = await self.flight_service.search_flights_period(origin, destination, months_ahead, promo_code)
